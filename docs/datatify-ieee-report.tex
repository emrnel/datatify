...
\begin{tikzpicture}[
  ...
  step/.style={rectangle, rounded corners=3pt, draw=black!70,
  ...
  \node[side, right=0.6cm of gemini_q, text width=1.9cm] (gemini_call)
        {\texttt{analyze\_character()}\\Gemini API};
...
\begin{tikzpicture}[
  ...
  tier/.style={rectangle, rounded corners=4pt, draw=black!80,
               ...
               align=center},
  step/.style={rectangle, rounded corners=3pt, draw=black!60,
  ...
  % ── Tier 1 steps ──────────────────────────────────────────────
  \node[step, below=0.4cm of py_h]  (py1) {Filter \& validate records};
  \node[step, below=0.25cm of py1]  (py2) {Single-pass \texttt{defaultdict}\\accumulator $O(N)$};
  \node[step, below=0.25cm of py2]  (py3) {Session detection\\(30-min gap heuristic)};
  \node[step, below=0.25cm of py3]  (py4) {15 behavioral metrics};

  % ── Tier 2 steps ──────────────────────────────────────────────
  \node[step, below=0.4cm of pd_h]  (pd1) {Load into \texttt{pd.DataFrame}};
  \node[step, below=0.25cm of pd1]  (pd2) {\texttt{groupBy().agg()}\\vectorized (NumPy SIMD)};
  \node[step, below=0.25cm of pd2]  (pd3) {\texttt{diff()} on timestamps\\for session boundaries};
  \node[step, below=0.25cm of pd3]  (pd4) {Same 15 metrics (cross-check)};

  % ── Tier 3 steps ──────────────────────────────────────────────
  \node[step, below=0.4cm of sp_h]  (sp1) {Parallelize RDD / DataFrame};
  \node[step, below=0.25cm of sp1]  (sp2) {Spark SQL \texttt{groupBy}\\Catalyst optimizer};
  \node[step, below=0.25cm of sp2]  (sp3) {Window fn session detection\\partitioned by user};
  \node[step, below=0.25cm of sp3]  (sp4) {MLlib K-Means clustering};
...
\subsection{Preprocessing and Optimal-$k$ Selection}
...
\begin{equation}
  N^* = \frac{\beta}{\gamma - \alpha} \approx \frac{8.0}{2.1\times10^{-5} - 6.5\times10^{-6}} \approx 540\,000 \text{ records}.
\end{equation}
